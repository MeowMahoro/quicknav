...
public abstract class AbstractSelectionPopup<W extends AbstractWidget> extends AbstractPopupScreen {
	...
	private @Nullable ItemList itemList;

	@Override
	public void init() {
		GridLayout.RowHelper adder = gridWidget.createRowHelper(2);
		addRenderableWidget(adder.addChild(itemList = new ItemList(300, (int) (height * 0.8f)), 2));
	...
	/**
	 * Rebuilds the widget grid using the current search text. Must be called on the render thread.
	 * Used when the underlying widget collection changes after the popup has been opened.
	 */
