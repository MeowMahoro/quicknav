 ...
 * <p>
 * This is a self-contained version of the Skyblocker item selection popup. The original fetches the full SkyBlock item
 * repository from the NEU repository; this standalone version ships a curated list of commonly used SkyBlock icons.
...
public class ItemSelectionPopup extends AbstractSelectionPopup<ItemSelectionPopup.ItemWidget> {
	...
	public ItemSelectionPopup(Screen backgroundScreen, Consumer<@Nullable ItemStack> onDone) {
		...
		this.items = SKULL_ICONS.stream()
				.map(pair -> createSkull(toTextureBase64(pair.right()), pair.left()))
				.map(ItemWidget::new)
				.toList();
	}
	...
	protected class ItemWidget extends AbstractWidget {
		...
		private ItemWidget(ItemStack stack) {
			super(0, 0, 20, 20, stack.getHoverName());
			item = stack;
