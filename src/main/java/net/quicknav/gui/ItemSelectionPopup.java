...
import net.quicknav.repo.ItemRepository;
import net.quicknav.repo.SkyblockItem;
import net.quicknav.utils.TextTransformer;
import org.jspecify.annotations.Nullable;

/**
 * A popup allowing the user to select a SkyBlock icon for their Quick Nav button.
 * <p>
 * A curated list of commonly used SkyBlock icons is always shown. If the NEU item repository has
 * finished loading (or finishes while the popup is open) the full SkyBlock item list is added as
 * well, so the popup works even when it is opened before the repository finishes downloading.
 * Any other item can still be chosen manually through the Item and Item Components config options.
 */
public class ItemSelectionPopup extends AbstractSelectionPopup<ItemSelectionPopup.ItemWidget> {

	private static final List<Pair<String, String>> SKULL_ICONS = List.of(
	...
	private final List<ItemWidget> items = new ArrayList<>();
	private boolean repositoryItemsAdded = false;
	...
	public ItemSelectionPopup(Screen backgroundScreen, Consumer<@Nullable ItemStack> onDone) {
		...
		SKULL_ICONS.forEach(pair -> items.add(new ItemWidget(createSkull(toTextureBase64(pair.right()), pair.left()), null)));
		if (ItemRepository.itemsImported()) {
			addRepositoryItems();
		} else {
			// The repository may still be downloading when the popup is opened; add the items once it is done.
			ItemRepository.runAfterImport(() -> Minecraft.getInstance().execute(this::addRepositoryItems), false);
		}
	}

	/**
	 * Adds every item of the NEU repository to the list. Idempotent; safe to call again if the
	 * repository is re-imported while the popup is open.
	 */
	private void addRepositoryItems() {
		if (repositoryItemsAdded) return;
		repositoryItemsAdded = true;
		ItemRepository.getItemsStream().forEach(skyblockItem -> items.add(new ItemWidget(null, skyblockItem)));
		refreshList();
	}

	private static ItemStack createSkull(String textureBase64, String name) {
		GameProfile profile = new GameProfile(UUID.randomUUID(), name, propertyMapWithTexture(textureBase64));
	...
	private static PropertyMap propertyMapWithTexture(String textureValue) {
	...
	private static String toTextureBase64(String textureUUID) {
	...
	protected class ItemWidget extends AbstractWidget {
		private final @Nullable ItemStack fixedItem;
		private final @Nullable SkyblockItem skyblockItem;
		private @Nullable ItemStack cachedStack;

		private ItemWidget(@Nullable ItemStack fixedItem, @Nullable SkyblockItem skyblockItem) {
			super(0, 0, 20, 20, fixedItem != null ? fixedItem.getHoverName() : TextTransformer.fromLegacy(skyblockItem.displayName()));
		...
		protected ItemStack getItem() {
			if (fixedItem != null) return fixedItem;
			// Lazy construction: building an ItemStack runs a data fixer upgrade, so only do it once per item.
			if (cachedStack == null) cachedStack = ItemRepository.getItemStack(skyblockItem);
			return cachedStack;
		}

		@Override
		protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
			graphics.item(getItem(), getX() + 2, getY() + 2);
