...
public interface FallbackedTexture<T> extends Supplier<T> {
	 ...
	 * @param atlas    the atlas to look in, pass null if the texture isn't in any atlas.
	...
	/**
	 * Registers a resource reload listener that updates all fallbacked textures after every resource reload.
	 */
	static void init() {
		Identifier id = QuickNavMod.id("fallback_texture_reloader");
		ResourceLoader.get(PackType.CLIENT_RESOURCES).registerReloadListener(id,
				(_, executor, preparationBarrier, reloadExecutor) -> CompletableFuture
						.supplyAsync(() -> Unit.INSTANCE, executor)
						.thenCompose(preparationBarrier::wait)
						.thenAcceptAsync(_ -> UPDATE_CALLBACKS.forEach(Runnable::run), reloadExecutor));
